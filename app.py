...
def dashboard():
    ...
    # Total Stocks is system-wide and is not affected by the current search/filter.
...
def change_password():
    ...
    ok, msg = AuthController.change_password_direct(
        session["username"], session.get("email", ""), old_password, new_password
    )
    flash(msg, "success" if ok else "danger")
    return redirect(url_for("dashboard"))
...
if __name__ == "__main__":
    # WEB ENTRY POINT:
    # The original desktop start_app() is intentionally NOT called.
    init_db()

    print()
    print("=" * 58)
    print(" NU HARDWARE INVENTORY - WEB PORTAL")
    print("=" * 58)
    print()
